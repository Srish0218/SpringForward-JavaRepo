...
public class Problem61_validScored {
    public static void main(String[] args) {
        ...
        System.out.println("Valid elements from list: ");
        List<Integer> validList = findValidScores(list);
        System.out.println(validList.isEmpty() ? "No valid score found" : "Valid Scores = " + validList);

    }
    public static List<Integer> findValidScores(List<Integer> list) {
        List<Integer> validScoreList = new ArrayList<>();
        for(Integer i : list){
            if(validScore(i)){
                validScoreList.add(i);
            }
        }
        return validScoreList;

    }
    public static boolean validScore(int i){
        return i <= 100 && i > 0;
    }
}
